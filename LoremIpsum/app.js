const form = document.getElementById('randomTextForm');
...
form.addEventListener('submit', function (event) {
  ...
  const randomParagraphs = generateRandomParagraphs(amount);
  generatedText.innerHTML = randomParagraphs;
});

function generateRandomParagraphs(count) {
  let result = '';
  if (count <= 0) {
    result += `<p>Please enter a valid input.</p>`;
  } else {
    const text = [
...
I just told you! You've killed me! Fry! Quit doing the right thing, you jerk! Michelle, I don't regret this, but I both rue and lament it. Morbo can't understand his teleprompter because he forgot how you say that letter that's shaped like a man wearing a hat.`,
    ...
    const shuffledText = text.sort(() => Math.random() - 0.5);

    for (let i = 0; i < count; i++) {
      result += `<p>${shuffledText[i % shuffledText.length]}</p>`;
    }
  }
  return result;
}
